...
def parse_items(text: str) -> list[dict]:
    ...
    for cost_i in cost_indices:
        ...
        if not name_line or name_line.lower() in {"blackpowder weapons", "black powder weapons"}:
            continue
...
def main() -> None:
    ...
    parser.add_argument("input", help="Path to a raw blackpowder weapons text file.")
    parser.add_argument(
        "--output",
        default="apps/items/data/standard-blackpowder.json",
    ...
    print(f"Wrote {len(items)} blackpowder weapons to {output_path}")
