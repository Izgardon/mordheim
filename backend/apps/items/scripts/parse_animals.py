...
def _parse_stat_block(lines: list[str], start_index: int) -> dict | None:
    """Parse the profile stat block (M WS BS S T W I A Ld)."""
    stats = None
    for i, line in enumerate(lines[start_index:], start=start_index):
        ...
        if re.match(r"^Profile\s+M\s+WS\s+BS\s+S\s+T\s+W\s+I\s+A\s+Ld", stripped, re.IGNORECASE):
            # Next non-empty line should have the values
            for j in range(i + 1, len(lines)):
                value_line = lines[j].strip()
                if not value_line:
                    continue
                # Parse the stat line (e.g., "Wardog	6	4	0	4	3	1	4	1	5")
                parts = value_line.split()
                if len(parts) >= 10:
                    # First part is the name, rest are stats
                    try:
                        stats = {
                            "M": parts[1],
                            "WS": parts[2],
...
def _collect_description(lines: list[str], start_index: int) -> str:
    ...
    for line in lines[start_index:]:
        ...
        # Stop at profile or special rules
        if lower.startswith(("profile", "special rules")):
            break
...
def _determine_subtype(name_line: str, description: str) -> str:
    """Determine if it's an attack animal or mount."""
    lower_name = name_line.lower()
    lower_desc = description.lower()

    # Attack animals
    if any(term in lower_name for term in ["wardog", "gnoblar"]):
        return "Attack Animal"

    # Mounts
    if any(term in lower_name for term in ["horse", "steed", "wolf", "boar", "spider", "nightmare", "rhinox", "mule", "temple dog", "cold one"]):
        return "Mount"

    # Check description
    if "mount" in lower_desc or "ridden" in lower_desc or "rider" in lower_desc:
        return "Mount"
...
def parse_items(text: str) -> list[dict]:
    ...
    for cost_i in cost_indices:
        ...
        if not name_line or name_line.lower() in {"attack animals", "ridden animals"}:
            continue
    ...
    for idx, meta in enumerate(item_meta):
        ...
        stats = _parse_stat_block(block, profile_index) if profile_index is not None else None
        ...
        subtype = _determine_subtype(name_line, description)
...
def main() -> None:
    ...
    parser.add_argument("input", help="Path to a raw animals text file.")
    parser.add_argument(
        "--output",
        default="apps/items/data/standard-animals.json",
    ...
    print(f"Wrote {len(items)} animals to {output_path}")
