...
type BattleMobileBottomBarConfig = Parameters<
  NonNullable<BattleLayoutContext["setBattleMobileBottomBar"]>
>[0];

type UseBattleMobileBottomBarParams = {
  isMobile: boolean;
  setBattleMobileBottomBar?: BattleLayoutContext["setBattleMobileBottomBar"];
  config: BattleMobileBottomBarConfig;
};

export function useBattleMobileBottomBar({
  isMobile,
  setBattleMobileBottomBar,
  config,
}: UseBattleMobileBottomBarParams) {
  useEffect(() => {
    if (!isMobile || !setBattleMobileBottomBar) {
      return;
    }

    setBattleMobileBottomBar(config);

    return () => {
      setBattleMobileBottomBar(null);
    };
  }, [config, isMobile, setBattleMobileBottomBar]);
}
