      ...
      >
        <div className="flex items-center justify-between gap-2">
          {config.leftAction ? (
            <Button
              size="sm"
              variant={config.leftAction.variant ?? "secondary"}
              onClick={config.leftAction.onClick}
              disabled={config.leftAction.disabled}
            >
              {config.leftAction.label}
            </Button>
          ) : (
            <div />
          )}
...
export default function BattleLayout() {
  ...
  return (
    <div className="relative min-h-full">
      ...
      <div className="px-2 pb-[calc(env(safe-area-inset-bottom,0px)+4.75rem)] pt-[calc(env(safe-area-inset-top,0px)+4.25rem)]">
