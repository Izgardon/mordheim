...
import HeroStatsGrid from "../../heroes/edit/HeroStatsGrid";
import SearchableDropdown from "../../heroes/edit/SearchableDropdown";
...
const getHenchmanHireCost = (group: HenchmenGroupFormEntry) => {
  const baseCost = toNumber(group.price);
  const xpCost = toNumber(group.xp) * 2;
  const henchmenCount = (group.henchmen ?? []).length;
  const itemCounts = (group.items ?? []).reduce<Record<number, { item: (typeof group.items)[number]; count: number }>>(
    (acc, item) => {
      if (acc[item.id]) {
        acc[item.id].count += 1;
      } else {
  ...
  const itemsCost = Object.values(itemCounts).reduce((sum, entry) => {
    const cost = Number(entry.item.cost ?? 0);
    const normalized = Number.isFinite(cost) ? cost : 0;
    let multiplier = 1;
    if (henchmenCount > 0 && entry.count >= henchmenCount * 2) {
      multiplier = 2;
    }
    return sum + normalized * multiplier;
  }, 0);
  return baseCost + itemsCost + xpCost;
};
