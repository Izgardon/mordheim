        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="flex min-h-[28px] flex-wrap items-end gap-3">
            ...
            <label className="flex flex-col items-center gap-1 text-xs text-foreground">
              Spc
              <Checkbox
                checked={hero.available_skills.Spc}
                onChange={(event) => {
            ...
            {hero.available_skills.Spc && (
              <TraitDropdown
                options={skillTypeOptions}
                selected={skillTypeOptions.filter((t) => hero.available_skills[t])}
                onSelect={(type) =>
                  onUpdate(index, (current) => ({
                    ...current,
                    available_skills: {
